...
public interface ValidatorUtil {
    ...
    static <T> void requireNumber(Number number, Supplier<? extends RuntimeException> exceptionProducer) {
    ...
    static void validateIfTradeIdIsNegativeOrZero(long tradeId, Supplier<? extends RuntimeException> exceptionProducer) {
        if (tradeId <= 0) {
            throw exceptionProducer.get();
        }
    }
